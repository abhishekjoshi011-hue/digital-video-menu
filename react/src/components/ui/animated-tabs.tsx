...
const AnimatedTabs = ({
  ...
  return (
    <div className={cn("w-full max-w-lg flex flex-col gap-y-1", className)}>
      <div className="grid grid-cols-3 gap-2 bg-[#11111198] bg-opacity-50 backdrop-blur-sm p-1 rounded-xl">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => {
            ...
            className={cn(
              "relative w-full px-2 py-2 text-sm font-medium rounded-lg text-white outline-none transition-colors whitespace-nowrap"
            )}
          >
            {activeTab === tab.id && (
              <motion.div
